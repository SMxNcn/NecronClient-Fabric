    ...
    init {
        on<GuiEvent.Render> {
            ...
            with (term) {
                ...
                when (type) {
                    TerminalTypes.RUBIX -> {
                        click(slotIndex, if (solution.count { it == slotIndex } >= 3) 1 else 2, true)
                        println("Click on $slotIndex [RUBIX]")
                    }
                    TerminalTypes.MELODY -> {
                        click(solution.find { it % 9 == 7 } ?: return@on, 2, true)
                        println("Click on $slotIndex [MELODY]")
                    }
                    else -> {
                        click(slotIndex, 2, true)
                        println("Click on $slotIndex []")
                    }
                }
            }
        }
