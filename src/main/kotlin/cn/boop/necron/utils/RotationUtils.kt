...
object RotationUtils {
    ...
    fun getClientRotation(): Rotation = Rotation(clientYaw, clientPitch)
    fun getServerRotation(): Rotation = Rotation(serverYaw, serverPitch)
    ...
    @JvmStatic
    fun update(rawYaw: Float, rawPitch: Float, target: Vec3?, aimSpeed: Float, serverAim: Boolean) {
        clientYaw = rawYaw
        clientPitch = rawPitch

        if (!initialized) {
            serverYaw = rawYaw
            serverPitch = rawPitch
            initialized = true
            return
        }

        if (serverAim && target != null) {
            val targetRot = vec3ToRotation(target)
            serverYaw = exponentialSmooth(serverYaw, targetRot.yaw, aimSpeed)
            serverPitch = exponentialSmooth(serverPitch, targetRot.pitch, aimSpeed)
        } else {
            serverYaw = clientYaw
